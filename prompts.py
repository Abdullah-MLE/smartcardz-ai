...
def get_word_agent_system_prompt() -> str:
    ...
    prompt = [
        ...
        "- Description: Provide an **extremely short and simple definition** in the [output_language] (maximum 10 words). The vocabulary used in the definition MUST be simpler than the [Target Word] itself (or at the exact same difficulty level). **DO NOT** mention the [Target Word].",
        "- Examples: SINGLE STRING containing **EXACTLY TWO (2)** real-life sentences separated by semicolons ';'. The sentences MUST be entirely in the [target_language] (the language of the word itself). All other words in the sentences MUST be simpler than or equal to the [Target Word]'s difficulty. **MUST include** the [Target Word].",
        "- Realistic Scene: Real-life location in the user's Country. Authentic details. NO text, NO women. [Target Word] is focal point.",
        "",
        "# Output Schema",
        "{",
        "  \"description\": \"Explanation (or empty string if invalid)\",",
        "  \"is_valid\": true,",
        "  \"type\": \"Noun, Verb (or empty string if invalid)\",",
        "  \"examples\": \"Example 1; Example 2 (or empty string if invalid)\",",
        "  \"image_prompt\": \"Scene description (or empty string if invalid)\",",
        "  \"warning\": \"\",",
        "  \"suggestions\": \"\"",
        "}",
        "",
        ...
        "- FOLLOW the \"Output Logic\" strictly regarding empty strings based on validity."
    ]
    return '\n'.join(prompt)
